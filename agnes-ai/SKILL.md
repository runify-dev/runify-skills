---
name: 视频生成(agnes供应商)
description: 基于 agnes-ai 的图像与视频生成技能。核心是一组**自由原子工具**（随调随用、互不依赖）：文生图、图生图、图片润色/重绘、图片理解与自检、文生视频、单图生视频（图片动画化）、多图视频/关键帧动画（多张参考图做平滑过渡、保持一致）、视频抽帧；支持给一张图就改图/润色/理解/生成视频，以及拿上一个产物接着做下一个。每步"一次调用产出一个产物"，可流式推前端、断点续跑、产物版本化可回退。**做短剧/小说转视频**时，由 agent 用这些原子工具自行编排（拆镜→逐镜出画→逐镜出视频→抽上一镜末帧续接保证连贯→合成），按提示词引导而非固定流水线脚本。当用户要：生成/修改/润色图片、理解或分析一张图、文生图、图生图、文生视频、图生视频、把图片做成视频、做短剧/短劇、小说转视频、把一张图动画化、视频续写/接续，或提到 agnes-ai 时，使用本技能。
metadata:
  runify:
