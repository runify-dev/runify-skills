---
name: 浏览器检索
description: 联网检索与网页正文抽取技能。一组随调随用、互不依赖的原子工具：search（给关键词/问题，返回若干条结果——标题/链接/摘要）、fetch（抓取某个网页并抽取正文，去掉脚本/导航/页脚等噪声，可输出纯文本或 markdown）、search_read（一步到位：先搜索再自动读取前 N 条结果的正文）。默认用 DuckDuckGo 搜索（免 API key，开箱即用），也可在技能设置里切换 Serper / Tavily 后端。纯 HTTP 抓取 + 解析、不渲染 JavaScript，无需安装浏览器。当用户要：联网搜索、查资料、找最新信息、读取某个网页内容、把网页转成文字/markdown、根据问题去网上搜并总结、检索某主题，或提到上网查/搜一下时，使用本技能。
metadata:
  runify:
