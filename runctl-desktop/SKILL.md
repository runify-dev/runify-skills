...
**核心：能预判的连续动作一口气做完，做完一"段"再截图核对。** 截图只发生在两种时刻：
**① 需要看清当前状态/定位某个控件时；② 做完一段动作要确认结果时。** 中间的可预判动作（尤其键盘）**别拆开、别逐个截图**。

**先建图（仅复杂界面才需要，一次）**：`runctl --json windows` 记下目标 id →
`runctl activate --window-id <ID>` → `runctl --json shot --window-id <ID> --grid --out "$RUNIFY_OUTPUT_DIR/ui.png"` →
让视觉给出**界面地图**（搜索框/输入框等关键控件的像素位置，照网格刻度记），记住复用。
> **简单任务**（发条消息、点个按钮）跳过建图，直接干。

**循环（直到 `done`）：**
1. **看清**（仅当需要）：`runctl --json shot --window-id <ID> --grid --out "$RUNIFY_OUTPUT_DIR/now.png"`（记 W、H）。
   位置已知（建过图/上一段刚确认）就别重复截图。
2. **想清这一段**：带着**具体目标**问视觉（模板见下），让它回 `{state, plan, done}`——
   `plan` 是**为推进目标接下来能一口气做的几步**（每步 `{action, xy?, text?}`）。位置未知才让它给 `xy`。
3. **成批做完这一段**：`activate` 确保焦点后，**连着执行 plan 里的几步、中间不截图**。例：
   `click 搜索框 → type "张少虎" → key enter`；或进会话后 `click 输入框 → type "消息" → key enter`。
   （键盘优先：发送/提交一律 `key enter`，不找按钮。位置已知的 click 直接做；**只有位置未知才回第 1 步截图定位**——别瞎点。）
