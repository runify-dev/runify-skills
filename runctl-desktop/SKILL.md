...
2. **想清这一段**：带着**具体目标**问视觉（模板见下），让它**只报感知**——
   `{state, targets, target_visible, blockers, done}`：`targets` 是目标相关控件的坐标（搜索框/输入框等），`target_visible` 是要找的对象在不在可见区。**动作顺序你自己排，别让视觉替你编排。**
3. **成批做完这一段**：`activate` 后据 `targets` 按**键盘优先**自己编排、连着做完、中间不截图。例：
   `target_visible=false` → `click 搜索框 → type "名字" → key enter` 进会话 → `click 输入框 → type "消息" → key enter`。
   （位置已知的 click 直接做；缺的坐标才回第 1 步截图问，别瞎点。键盘优先见上节。）
...
### 给视觉的提问：目标性提问，视觉只报坐标、别替你排动作

**带上具体目标，让视觉只回"状态 + 相关控件坐标 + 目标可不可见"**，动作顺序你自己按键盘优先编排。
**不用告诉它尺寸——网格上已标坐标，照格线读即可：**

```
这是 <应用> 窗口截图，已叠坐标网格（刻度=图像像素，照格线读坐标）。
我的目标：<具体目标，如"跟张少虎发一条消息：晚上一起吃饭吗">。
只看这张图，用 JSON 回答：
- state          一句话当前状态（在哪个界面/哪个会话）
- targets        为达成目标会用到的控件及像素坐标，如 {搜索框:[x,y], 输入框:[x,y]}（看不到的给 null）
- target_visible 要找的对象(如"张少虎")在当前可见区吗？true/false（false → 用搜索框找）
- blockers       要先处理的弹窗/登录/报错（无则 none）
- done           目标是否已完成 true/false
```

**例：跟张少虎聊天**（视觉报坐标，agent 自己成批编排）——
1. 截一张 → 问模板 → 视觉回 `target_visible=false, targets={搜索框:[100,40], 输入框:[600,1200]}`
   → agent 据此连着做完：`click 搜索框 → type "张少虎" → key enter → click 输入框 → type "晚上一起吃饭吗" → key enter`。
2. 截一张核对"消息发出了吗" → `done=true` 收尾。

→ 全程 **2 次截图**（定位一次、核对一次）：视觉只管"哪儿有什么"，编排和键盘优先由 agent 把控。
